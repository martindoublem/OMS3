...
!************************************************************************
! @Description("plant soil water availability calculation")
! @VersionInfo("$Id$")
...
! @Execute
     ...
     &    SOIL_INP, SOIL_INP_LEN,                         !Input
     &    IRRIG_INP, IRRIG_INP_LEN,                       !Input
     &    SW_OUT, SW_OUT_LEN,                             !Input
     &    DYN, DYN_LEN)                                   !Control
...
!-----------------------------------------------------------------------
      ...
      ! @In
      REAL(C_FLOAT) :: LAI, RAIN, SRAD, TMAX, TMIN
      
      ! @Out
      REAL(C_FLOAT) :: SWFAC1, SWFAC2
      
      ! @In
      CHARACTER(kind = C_CHAR, len = SOIL_INP_LEN) :: SOIL_INP
      INTEGER(C_INT), VALUE :: SOIL_INP_LEN
      
      ! @In
      CHARACTER(kind = C_CHAR, len = IRRIG_INP_LEN) :: IRRIG_INP
      INTEGER(C_INT), VALUE :: IRRIG_INP_LEN
      
      ! @In
      CHARACTER(kind = C_CHAR, len = SW_OUT_LEN) :: SW_OUT
      INTEGER(C_INT), VALUE :: SW_OUT_LEN
      ...
      INTEGER   DATE
      REAL      SWC,INF,IRR,ROF,ESa,EPa,DRNp
      REAL      DRN,DP,WPp,FCp,STp,WP,FC,ST,ESp,EPp,ETp
...
!        INQUIRE(unit=3, opened=ISOPEN)
        IF (ISOPEN .eqv. .FALSE.) THEN  
            ! DO INIT 
            ISOPEN = .TRUE.
        OPEN(3,FILE=SOIL_INP,STATUS='UNKNOWN')
        OPEN(10,FILE=SW_OUT,  STATUS='REPLACE')
        OPEN(11,FILE=IRRIG_INP,STATUS='UNKNOWN')
...
!        WRITE(10,44) DOY, SRAD, TMAX, TMIN, RAIN, IRR, SWFAC1

    

        WRITE(10,46) DOY+1, SRAD, TMAX, TMIN, RAIN, IRR, ROF, INF, DRN, 
     &    ETP, ESa, EPa, SWC, SWC/DP, SWFAC1, SWFAC2


   40   FORMAT(I5,3F8.1,9F8.2,3F8.3)
   41   FORMAT("@T,","CSM")
   42   FORMAT("created_at, ","Tue Oct 25 11:46:24 MDT 2011")
   43   FORMAT("date_format, yyyy-DDD")
   44   FORMAT( "@H,DOY, SRAD, TMAX, TMIN, RAIN, IRR, ROF, INF, DRN, 
     &ETP, ESa, EPa, SWC, SWC/DP, SWFAC1, SWFAC2")
   45   FORMAT("type,Integer,Real,Real,Real,Real,Real,Real,Real,Real,Real,Real,
     &  Real,Real,Real,Real,Real")
   46   FORMAT (",","2011-",I3.3,15(",",F8.3))  
   47   FORMAT (",",I3,15(",",F8.3))
!************************************************************************
!************************************************************************
!     CLOSE
!************************************************************************

...
public abstract class AbstractDoubleList extends AbstractList implements DoubleBufferConsumer {
     ...
     * before the specified position into the receiver. Shifts the element currently at that position
     ...
     * to the right. This method must set the new size to be size() + length.
     ...
     * The receiver must be sorted (as by the sort method) prior to making this call. If
     * it is not sorted, the results are undefined: in particular, the call may enter an
     * infinite loop. If the receiver contains multiple elements equal to the specified 
     ...
     *	    element greater than the key, or receiver.size(), if all elements in the receiver
     *      are less than the specified key. Note that this guarantees that the return value
     ...
     *      greater than the key, or receiver.size(), if all elements in the receiver are
     *      less than the specified key. Note that this guarantees that the return value will
     ...
     * capacity. WARNING: For efficiency reasons and to keep memory usage low, this method may decide
     * not to copy the array. So if subsequently you modify the returned array directly via the 
     ...
     * specified Object is also an ArrayList of the same type, both Lists have the same size,
     * and all corresponding pairs of elements in the two Lists are identical. In other words,
     ...
     * checked before the loop and need no be rechecked within the loop. However, when desperate,
     * you can give this method public visibility in subclasses.
     * @param index index of element to return.
     */
     ...
     * guaranteed n*log(n) performance, and can approach linear performance on nearly sorted
     * lists. You should never call this method unless you are sure that this particular sorting
     * algorithm is the right one for your data set. It is generally better to call sort() or
     * sortFromTo(...) instead, because those methods automatically choose the best sorting algorithm.
    ...
    /*
     * Sorts the receiver according to the order induced by the specified comparator. All
     * elements in the range must be mutually comparable by the specified comparator (that
     * is, c.compare(e1, e2) must not throw a ClassCastException for any elements e1 and
     * e2 in the range). This sort is guaranteed to be stable: equal elements will not be
     * reordered as a result of the sort. The sorting algorithm is a modified mergesort
     * (in which the merge is omitted if the highest element in the low sublist is less
     * than the lowest element in the high sublist). This algorithm offers guaranteed
    ...
    /*
     * Sorts the specified range of the receiver into ascending numerical order. The 
     * sorting algorithm is a tuned quicksort, adapted from Jon L. Bentley and M. Douglas
     ...
     * You should never call this method unless you are sure that this particular sorting
     * algorithm is the right one for your data set. It is generally better to call sort()
     * or sortFromTo(...) instead, because those methods automatically choose the best
     * sorting algorithm.
     * @param from the index of the first element (inclusive) to be sorted.
     * @param to the index of the last element (inclusive) to be sorted.
     * @exception IndexOutOfBoundsException index is out of range (size() > 0 &&
     ...
     * e1 and e2 in the range). The sorting algorithm is a tuned quicksort, adapted from
     ...
     * algorithm offers n*log(n) performance on many data sets that cause other quicksorts 
    ...
    /*
     * Removes from the receiver all elements whose index is between from, inclusive and
     * to, inclusive.  Shifts any succeeding elements to the left (reduces their index).
     ...
     * another list. Replaces elements in the receiver, between from (inclusive) and to 
     * (inclusive), with elements of other, starting from otherFrom (inclusive).
     ...
     * between otherFrom and otherTo. Powerful (and tricky) method! Both parts need not be of
     * the same size (part A can both be smaller or larger than part B). Parts may overlap.
     * Receiver and other list may (but most not) be identical. If from > to, then inserts
     * other part before from.
    ...
    /*
     * Replaces the part of the receiver starting at from(inclusive) with all the elements
     * of the specified collection. Does not alter the size of the receiver. Replaces exactly
     ...
     * where bounds are explicitly checked before the loop and need no be rechecked within the loop.
     * However, when desperately, you can give this method public visibility in subclasses.
     * @param index index of element to replace.
     * @param element element to be stored at the specified position.
     */
	...
	Uniform gen = new Uniform(Uniform.makeDefaultGenerator());
	for (int i = from; i < to; i++) { 
    ...
    /*
     * Returns a string representation of the receiver, containing the String representation
     * of each element.
     */

...
public abstract class AbstractIntList extends AbstractList implements IntBufferConsumer {
    ...
    */
     ...
     * @return index of the search key, if it is contained in the receiver; otherwise, 
     *      (-(insertion point</i>) - 1). The insertion point is defined as the the point at
     ...
     * corresponding pairs of elements in the two Lists are identical. In other words, two Lists
     ...
     * @param procedure to be applied. Stops iteration if the procedure returns false, otherwise continues. 
     ...
     * @param from the leftmost search position, inclusive.
     * @param to the rightmost search position, inclusive.
     * @return  the index of the first occurrence of the element in the receiver; returns -1 if
     ...
     * modified mergesort (in which the merge is omitted if the highest element in the low sublist
     * is less than the lowest element in the high sublist). This algorithm offers guaranteed
     * n*log(n) performance, and can approach linear performance on nearly sorted lists. You should
     * never call this method unless you are sure that this particular sorting algorithm is the
     * right one for your data set. It is generally better to call sort() or sortFromTo(...) instead,
     * because those methods automatically choose the best sorting algorithm.
     ...
     * the sort. The sorting algorithm is a modified mergesort (in which the merge is omitted if
     * the highest element in the low sublist is less than the lowest element in the high sublist).
     * This algorithm offers guaranteed n*log(n) performance, and can approach linear performance 
     ...
     * 1249-1265 (November 1993). This algorithm offers n*log(n) performance on many data sets 
     * that cause other quicksorts to degrade to quadratic performance. You should never call
     * this method unless you are sure that this particular sorting algorithm is the right one
     * for your data set. It is generally better to call sort() or sortFromTo(...) instead,
     * because those methods automatically choose the best sorting algorithm.
     * @param from the index of the first element (inclusive) to be sorted.
     * @param to the index of the last element (inclusive) to be sorted.
     * @exception IndexOutOfBoundsException index is out of range (size() > 0 
    ...
    /*
     * Replaces elements in the receiver, between from (inclusive) and to (inclusive), with elements
     * of other, starting from otherFrom (inclusive).
     ...
     * Receiver and other list may (but most not) be identical. If from > to, then inserts other
     * part before from.
     ...
     * access invalid indexes without throwing any exception! You should only use this method when
     * you are absolutely sure that the index is within bounds. Precondition (unchecked):

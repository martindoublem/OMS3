...
/*
 * This class implements matrices of bits of arbitrary dimensions. Basic facilities for bits
 * operations, multiplications and exponentiations are provided.
 */
public class BitMatrix implements Serializable, Cloneable  {
    ...
    /*
     * Creates a new BitMatrix using the data in rows. Each of the BitVectors will be one of the
     * rows of the BitMatrix.
    ...
    /*
     * Creates a new BitMatrix with r rows and c columns using the data in data. Note that the 
     * orders of the bits for the rows are using the same order than for the BitVector. This does
     * mean that the first bit is the lowest order bit of the last int in the row and the last
     * bit is the highest order bit of the first int int the row.
    ...
    /*
     * Verifies that 'this' and 'that' are strictly identical. They must both have the same
     * dimensions and data.
    ...
    /*
     * Creates a String containing all the data of the BitMatrix. The result is displayed in a 
     * matrix form, with each row being put on a different line. Note that the bit at (0,0) is
     * at the upper left of the matrix, while the bit at (0) in a BitVector is the least 
     * significant bit.
    ...
    public String toString()  {
        ...
        /* Displays the bitstream in the reverse order given by the BitVector so that the bit at
         *(0,0) would be the highest
         */
    ...
    /*
     * Creates a String containing all the data of the BitMatrix. The data is displayed in the same
     * format as are the int[][] in Java code. This allows the user to print the representation of
     * a BitMatrix to be put, directly in the source code, in the constructor BitMatrix(int[][], int, int).
   ...
   /*
    * Returns the value of the bit in the specified row and column. If the value is 1, return true.
    ...
    * @exception IndexOutOfBoundsException if the selected bit would be outside the BitMatrix
    */
    ...
    /*
     * Changes the value of the bit in the specified row and column. If value is true, changes it
     * to 1. If value is false changes it to 0.
     * @param row the row of the selected bit
     * @param column the column of the selected bit
    ...
    /*
     * Returns the BitMatrix resulting from the application of the NOT operator on the original 
     * BitMatrix. The effect is to swap all the bits of the BitMatrix, turning all 0's into 1's
     * and all 1's into 0's.
    ...
    /*
     * Returns the BitMatrix resulting from the application of the AND operator on the original 
     * BitMatrix and 'that'. Only bits which were at 1 in both BitMatrix are set at 1 in the
     * result. All others are set to 0.
     * @param that the second operand of the AND operator
     * @return the result of the AND operator
     * @exception IncompatibleDimensionException if the two BitMatrix are not of the same dimensions
     */
    public BitMatrix and (BitMatrix that) {
        if(this.c != that.c || this.r != that.r) {
            throw new IncompatibleDimensionException("Both matrices must have the same dimension. " +
                    "this is a " + this.r + "x" + this.c + " matrix " + "while that is a " + that.r +
                    "x" + that.c + " matrix.");
        }
        BitMatrix result = new BitMatrix(this);
        for(int i = 0; i < r; i++) {
    ...
    /*
     * Returns the BitMatrix resulting from the application of the OR operator on the original BitMatrix
     * and 'that'. Only bits which were at 0 in both BitMatrix are set at 0 in the result. All others 
     * are set to 1.
    ...
    /*
     * Returns the BitMatrix resulting from the application of the XOR operator on the original BitMatrix
     * and 'that'. Only bits which were at 1 in only one of the two BitMatrix are set at 1 in the result.
    ...
    /*
     * Multiplies the column BitVector by a BitMatrix and returns the result. The result is A*v, where
    ...
    /*
     * Multiplies vect, seen as a column BitVector, by a BitMatrix. (See BitVector to see the
     * conversion between int and BitVector.) The result is A*v, where A is the BitMatrix, and
     * v is the BitVector.
     * @param vect the vector to multiply
    ...
    /*
     * Multiplies two BitMatrix's together. The result is A*B, where A is the 'this BitMatrix' and B is
     * the 'that BitMatrix'.
     * @param that the other BitMatrix to multiply
     * @return the result of the multiplication
     * @exception IncompatibleDimensionException if the number of columns of the first BitMatrix> is not
     * equal to the number of rows of the second BitMatrix
     */

...
public interface IntComparator {
     ...
     * the second. The implementor must ensure that sgn(compare(x, y)) == -sgn(compare(y, x))
     * for all x and y. (This implies that compare(x, y) must throw an exception if and only
     * if compare(y, x) throws an exception.) The implementor must also ensure that the
     * relation is transitive: ((compare(x, y) > 0) && (compare(y, z) > 0)) implies
     * compare(x, z) > 0.<p> Finally, the implementer must ensure that compare(x, y) == 0
     ...
     * ordering as this comparator. Thus, comp1.equals(comp2) implies that 
     * sgn(comp1.compare(o1, o2)) == sgn(comp2.compare(o1, o2)) for every element o1
     * and o2. Note that it is always safe not to override Object.equals(Object). However, 

...
public class ComponentAccess {
    ...
    Map<String, Access> inGrs = new LinkedHashMap<>();
    Map<String, Access> outs = new LinkedHashMap<>();
    Map<String, Access> outGrs = new LinkedHashMap<>();
    ...
    ComponentAccess(Object comp, Notification ens) {
        ...
        findAll(comp, ins, outs, inGrs, outGrs, ens);
    }
    ...
    public Collection<Access> inputsGraph() {
        return inGrs.values();
    }
    ...
    public Collection<Access> outputsGraph() {
        return outGrs.values();
    }
    ...
    public Access inputGraph(String field) {
        return inGrs.get(field);
    }
    ...
    public Access outputGraph(String field) {
        return outGrs.get(field);
    }
    ...
    final void exec() throws ComponentException {
        try {
            ...
            // synchonized in()
            for (Access a : inGrs.values()) {     // wait for all inputs to arrive
                if (a.getField().getName().equals("net3subtrees")) {
                    System.out.println(a.getField().getName());
                    a.in();
                }
                if (a.getClass() == FieldAccess.class) {
                    a.in();
            ...
            // unsynchronized out
            for (Access a : outGrs.values()) {    // notify for output.
                if (a.getClass() == FieldObjectAccess.class || a.getClass() == AsyncFieldAccess.class) {
                    a.out();
                }
            }
    ...
    private static void findAll(Object cmp, Map<String, Access> ins, Map<String, Access> outs, Map<String, Access> inGrs, Map<String, Access> outGrs, Notification ens) {
        Class cmpClass = cmp.getClass();
        Class infoClass = infoClass(cmpClass);
        for (Field f : infoClass.getFields()) {
            try {
                ...
                if (f.getAnnotation(InNode.class) != null) {
                    inGrs.put(f.getName(), new FieldAccess(cmp, cmpClass.getField(f.getName()), ens));
                }
                if (f.getAnnotation(Out.class) != null) {
                    outs.put(f.getName(), new FieldAccess(cmp, cmpClass.getField(f.getName()), ens));
                }
                if (f.getAnnotation(OutNode.class) != null) {
                    outGrs.put(f.getName(), new FieldAccess(cmp, cmpClass.getField(f.getName()), ens));
                }
            } catch (Exception ex) {

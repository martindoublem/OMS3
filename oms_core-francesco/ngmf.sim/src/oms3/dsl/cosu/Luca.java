...
public class Luca extends AbstractSimulation {
    ...
    public Object run() throws Exception {
        ...
        if (getGraphStructure() != null) {
            summary.setModelInfo(rounds, steps.size(), getGraphStructure().getClassname(),
                getGraphStructure().getParams(), calib_start, startTime, endTime, outFolder);
        } else {
...
//            System.out.println("post Step " + step.getName() + "->" + destStep.getName());
    ...
    final class ModelExecution implements ExecutionHandle {
        ...
        ModelExecution() throws IOException {
            ...
            if (getGraphStructure() != null) {
                csParameter = getGraphStructure().getParameter();
            } else {
        ...
        private void executeGraph(Step.Data step) throws Exception {

            writeParameterFile(step);

            if (setpath == null || setpath.isEmpty()) {
                setpath = getGraphStructure().newModelComponent();
            }

            int availProc = Runtime.getRuntime().availableProcessors()/2;
            ...
            executor.shutdown();
            clearMem();// is it required????????

        }

        private void executeModel(Step.Data step) throws IOException, Exception {
    ...
    private class RunSimulations {
        ...
        private class ParallelSim implements Runnable {
            ...
            public void run() {
                try {
                    empty = setpath.isEmpty(); // AWFUL!!!!
                    if (simpath == null) {
                        simpath = "./simulation/";
                    }
                    UnifiedParams par = getParameter();
                    while (!empty) {
                        String index = runSim();
                        empty = setpath.isEmpty(); // AWFUL!!!!
                        if (index != null) {
                            ...
                            String simtorun;
                            simtorun = simpath + index + ".sim";
                            comp = CLI.groovy(simtorun, log.getLevel().toString());

                            Sim tmpComp = (Sim) comp;

                            Boolean localCalibrate = tmpComp.getCalibrate();
                            if (localCalibrate == null && properties != null) {
                                localCalibrate = properties.get(index, "calibrate");
                            }
                            tmpComp.setNode(index);
                            tmpComp.setChildren(graph.getChildren(index));

                            if (localCalibrate != null && localCalibrate) {
...
//}                
        ...
        void writeParameterCopy(Step step, int round) throws FileNotFoundException {
            ...
            if (getGraphStructure() != null) {
                for (Params param : getGraphStructure().getParams()) {
            ...
            if (getGraphStructure() != null && getGraphStructure().getClassname() != null) {
                csp.getInfo().put("model_class", getGraphStructure().getClassname());
            } else if (getModelElement().getClassname() != null) {

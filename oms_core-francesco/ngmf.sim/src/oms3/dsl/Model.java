...
public class Model implements Buildable {
    ...
    KVPContainer grIn = new KVPContainer();
    KVPContainer grOut = new KVPContainer();
    ...
    public Buildable create(Object name, Object value) {
        if (name.equals("parameter")) {
            ...
            params.add(p);
            return p;
        } else if (name.equals("inFluxes")) {
            return grIn;
        } else if (name.equals("outFluxes")) {
            return grOut;
    ...
    void setChildren(Set<String> parents) {
    ...
    public Object newModelComponent(String node, Set<String> children) throws Exception {
...
//            }
        ...
        if (grOut.hasEntries()) {
            kvpGraphOutExpand(b, grOut.entries, "graph_out2out");
        }

        if (grIn.hasEntries()) {
            kvpGraphInExpand(b, grIn.entries, "graph_in2in");
        }
        // out2in
    ...
    void kvpGraphOutExpand(StringBuilder b, List<KVP> l, String method) {
        for (KVP c : l) {
            String key = c.getKey().trim();
            String val = c.getValue().toString().trim();
            ...
            String[] from = key.split("\\.");
            if (from.length != 2) {
                String msg = "Invalid @OutNode reference: '" + c.getKey();
                msg += " for node: " + node + ". Check Out Fluxes definition";
                throw new RuntimeException(msg);
            }

            b.append("  " + method + "(\"" + node + "\", \"" + from[1] + "\", " + from[0] + ");\n");

        }
    }

    void kvpGraphInExpand(StringBuilder b, List<KVP> l, String method) {
        for (KVP c : l) {
            String key = c.getKey().trim();
            String val = c.getValue().toString().trim();
            ...
            // TODO: add a check if the node retrieving data is actually connected to node is retrieving data from
            String[] keys = key.split("\\.");
            String from_node = keys[0];
            String from_field = keys[1];

            if (!from_node.equals("0") && parents != null) checkFromNode(from_node);
            String[] t = val.split("\\s+");    // multiple @In
            for (String kvp : t) {
                String to_obj = kvp;           // default target is just object name 
                String to_field = from[1];     // same as @Out
                ...
                if (to_field.equals("net3subtrees")) {
                    // this method is going to pick from subtrees datastructure
                    b.append("  graph_subtrees" + "(\"" + node + "\", " + to_obj + ", \"" + to_field + "\");\n");
                } else {
                    b.append("  " + method + "(\"" + from_node + "\", \"" + from_field + "\", " + to_obj + ", \"" + to_field + "\");\n");
                }
            }
        }
    }

    private void checkFromNode(String from_node) {
        if (from_node != null && !parents.contains(from_node)) {
            String msg = "Node " + node + " is not connected to " + from_node;

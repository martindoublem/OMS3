/*
 * $Id: SoilLayerN.java 1050 2010-03-08 18:03:03Z ascough $
 *
...
@Keywords
    ("Utilities")
@SourceInfo
    ("$HeadURL: http://svn.javaforge.com/svn/oms/branches/oms3.prj.ages-w/src/soilWater/SoilLayerN.java $")
@VersionInfo
    ("$Id: SoilLayerN.java 1050 2010-03-08 18:03:03Z ascough $")
@License
    ("http://www.gnu.org/licenses/gpl-2.0.html")
    
 public class SoilLayerN  {
    private static final Logger log =
            Logger.getLogger("oms3.model." + SoilLayerN.class.getSimpleName());
...
// In Out
    ...
    double[] NH4_Poolvals;
    double[] N_activ_poolvals;
    double[] N_stabel_poolvals;
    double[] N_residue_pool_freshvals;
    double[] Residue_poolvals;
    double[] interflowNvals;
    double[] percoNvals;
    double[] interflowNabsvals;
...
//        double[] NO3_Poolvals = new double[layer];
        ...
        if (NH4_Poolvals == null) {
            NH4_Poolvals = new double[layer];
            N_activ_poolvals = new double[layer];
            N_stabel_poolvals = new double[layer];
            N_residue_pool_freshvals = new double[layer];
            Residue_poolvals = new double[layer];
            interflowNvals = new double[layer];
            percoNvals = new double[layer];
            interflowNabsvals = new double[layer];
            percoNabsvals = new double[layer];
            ConcN_mobile = new double[layer];
...
//        double[] ConcN_mobile = new double[layer];
        ...
        while (i < layer) {
            ...
            gamma_temp = 0.9 * (runSoil_Temp_Layer /
                    (runSoil_Temp_Layer * Math.exp(9.93 - 0.312 * runSoil_Temp_Layer))) + 0.1;
            ...
            runN_stabel_pool += Hum_trans;
            if (runN_stabel_pool < 0) {
                runN_stabel_pool = 0;
            }

            runN_activ_pool -=  Hum_trans;
            if (runN_activ_pool < 0) {
                runN_activ_pool = 0;
            }

            Hum_act_min = calc_Hum_act_min();
            runN_activ_pool -=  Hum_act_min;
            if (runN_activ_pool < 0) {
                runN_activ_pool = 0;
            }

            if (i < 1) {
                runResidue_pool +=  Addresidue_pool + (fertorgNfresh * 10);
                ...
                runResidue_pool -= (delta_ntr * runResidue_pool);
                ...
                runN_stabel_pool +=  fertstableorg;
                Nactiverespool = 0.2 * (delta_ntr * runN_residue_pool_fresh);
                runN_activ_pool += fertorgNactive + Nactiverespool;

                if (runN_activ_pool < 0) {
                    runN_activ_pool = 0;
                }
                NO3respool = 0.8 * (delta_ntr * runN_residue_pool_fresh);
                runNO3_Pool +=  sum_Nupmove + fertNO3 + a_deposition + runnitri_trans + Hum_act_min + runinterflowN_in + runsurfaceN_in + NO3respool;
...
//                System.out.println(time.get(time.DAY_OF_YEAR) + " runNO3_Pool " + runNO3_Pool + " sum_Nupmove "+ sum_Nupmove + " fertNO3 "+ fertNO3 + " a_deposition "+ a_deposition + " runnitri_trans "+ runnitri_trans +" runinterflowN_in "+ runinterflowN_in +" runsurfaceN_in "+ runsurfaceN_in +" NO3respool "+ NO3respool);
                ...
                runNO3_Pool -=  rundenit_trans;
                if (runNO3_Pool < 0) {
                    runNO3_Pool = 0;
                }
                runN_residue_pool_fresh -= (delta_ntr * runN_residue_pool_fresh);
                if (runN_residue_pool_fresh < 0) {
                    runN_residue_pool_fresh = 0;
                }
            } else {
                runNO3_Pool += runnitri_trans + runinterflowN_in + percoNvals[i - 1] + Hum_act_min;
                rundenit_trans = calc_denitrification();
                runNO3_Pool -= rundenit_trans;
                if (runNO3_Pool < 0) {
                    runNO3_Pool = 0;
            ...
            if (i == 0) {
                ...
                runNO3_Pool -=  runsurfaceN;
            }
            runinterflowN = calc_interflowN(i);
            runNO3_Pool -= runinterflowN;
            runpercoN = calc_percoN(i);
            runNO3_Pool -= runpercoN;
            ...
            Sumdenit_trans += rundenit_trans;
            Sumnitri_trans += runnitri_trans;
            i++;
        }
...
//        }
    ...
    private double[] calc_plantuptake() {
        ...
        double bioNact = 0;
        bioNact = BioNAct + runactN_up;//EQUIV: BioNAct = BioNAct + (BioNOpt - BioNAct) + [more Nitrogen]. Result: Actual N content higher than Opt N content
        actnup = runactN_up;
        BioNAct = bioNact;
...
//        double concN_mobile = 0;
    ...
    private double calc_percoN(int i) {
     ...
     public static void main(String[] args) {
        oms3.util.Components.explore(new SoilLayerN());
    }
}

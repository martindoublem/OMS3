...
import optas.utils.ArrayColumnComparator;
import java.util.*;
    ...
    public Object[] mocom(int s, double minn[], double maxn[], int MaxIter) throws SampleLimitException, ObjectiveAchievedException{
        ...
        while (Rmax > 1) {
            ...
            if (MaxIter < loopcounter) {
                ...
                log(("MAXIMUM_NUMBER_OF_LOOPS_HAS_REACHED"));
                log("********************************************");
                c = 0;
                String out = ("Rank")+"\t";
...
//                    out += this.parameterNames[i] + "\t";
                ...
                c = 0;
                log(("Number of model runs: ") + this.icall);
                out = ("Rank")+"\t";
                for (int i = 0; i < n; i++) {
//                    out+=this.parameterNames[i] + "\t";
                }

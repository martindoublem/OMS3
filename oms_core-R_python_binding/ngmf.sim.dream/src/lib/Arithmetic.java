...
public class Arithmetic extends Constants {
    ...
    */
     ...
     * Coefficients are stored in reverse order, i.e. the zero order term is last in the array.
     * Note N is the number of coefficients, not the order. If coefficients are for the interval
     * a to b, x must have been transformed to x -> 2(2x - b - a)/(b-a) before entering the routine.
     * This maps x from (a, b) to (-1, 1), over which the Chebyshev polynomials are defined. If 
     * the coefficients are for the inverted interval, in which (a, b) is mapped to (1/b, 1/a),
     * the transformation required is x -> 2(2ab/x - b - a)/(b-a). If b is infinity, this becomes
     * x -> 4a/x - 1.
     * SPEED: Taking advantage of the recurrence properties of the Chebyshev polynomials, the routine
    ...
    /*
     * Returns the largest long <= value. Examples:
     * 1.0 -> 1, 1.2 -> 1, 1.9 -> 1
     * 2.0 -> 2, 2.2 -> 2, 2.9 -> 2
    ...
    */
    ...
    /*
     * Returns log(k!). Tries to avoid overflows. For k<30, simply looks up a table in O(1).
     * For k>=30 uses Stirling's approximation.
     * @param k must hold k >= 0.
     */
    ...
    /*
     * Returns the Stirling Correction. Correction term of the Stirling approximation for log(k!)
